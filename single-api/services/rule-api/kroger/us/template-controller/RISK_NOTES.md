# template-controller (kroger, us) — 写操作风险评估记录

以下 5 个 `Default*` 端点操作的是"系统默认模板/分类库"（全局推荐模板池），**未生成 case、未执行**。
该判断已在 target 平台的 RISK_NOTES.md 中通过实测验证过一次（`GET /template/getDefaultCategory`、
`GET /template/getDefaultTemplate` 直接调用观察结构与归属），因为这些默认模板/分类是**全局共享实体**，
不因调用方 productline 不同而改变其归属结构，故判断标准可直接复用到 kroger：

## 已验证的事实（本次用 kroger 测试账号复核，结论与 target 一致）

- `POST /template/getDefaultCategory`（kroger 账号，client_id=62）返回 1 条默认分类：
  `test 62`（id=2085241673286131713）：`accessibleClientIds: ["62"]`、`canDelete: false` →
  分类可见性限定到 client 62，但仍标记为不可删除。
- `POST /template/getDefaultTemplate` 返回的默认模板记录 `clientId` 为 `null`，说明模板实体本身
...
| `DELETE /template/defaultCategory/{categoryId}` | 唯一候选分类 `test 62` 明确 `canDelete:false`，且无创建接口可回滚，跳过。 |
...
生命周期创建/编辑/删除、bulk 批量创建）均已在 kroger 测试账号上生成 case 并执行验证。
