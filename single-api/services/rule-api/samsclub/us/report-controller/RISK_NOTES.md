# report-controller (samsclub, us) — 风险标注 / 跳过说明
...
`productLine` 字段，但抽取 ES 真实样本后发现请求体实际都携带 `productLine`（与 kroger/chewy 平台
一致的运行时字段，文档未登记但可据此做平台归因）。按 `body.productLine.keyword: samsclub` 过滤，
近 90 天命中数均为 **0**：
...
即 samsclub 平台从未有客户配置过 Profitero 品牌报表数据源（`schema: DATAMART_xxx` 与具体客户的数据
仓库绑定，samsclub 下无任何客户开通）。按铁律「ES 无流量禁止生成 cases.json、禁止执行」，以上 10 个
接口本次未生成 case、未执行，仅在 Excel 标注「ES 无流量」。
...
两接口均无 Swagger 声明参数，ES 近 90 天样本（各 90~106 条）显示其 `queryString` 均为
`env=k8s-Production-pacvue-custom-job-<pod名>`，即调用方是 Kubernetes 生产环境的**内部定时任务/
custom-job Pod**，而非用户交互触发的前端请求（与 kroger/chewy 平台观察到的模式完全一致，属于
平台无关的运维基础设施行为）。
